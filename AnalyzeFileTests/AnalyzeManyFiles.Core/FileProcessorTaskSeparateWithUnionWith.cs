...
public class FileProcessorTaskSeparateWithUnionWith : IFileProcessor
{
    private readonly IAnalyzeFileStrategy _analyzeFileStrategy;

    public FileProcessorTaskSeparateWithUnionWith(IAnalyzeFileStrategy analyzeFileStrategy)
    {
    ...
    public async Task<List<(string, string)>> AnalyzeFiles(string pathToTodayReport, string pathToYesterdayReport)
    {
        ...
        mergedResults.UnionWith(result1);
